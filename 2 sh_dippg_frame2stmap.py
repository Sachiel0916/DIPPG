...
train_dataset = []
data_total_xls = xlrd.open_workbook_xls('./08_NIRP_Car975_Dataset/Car_975_data_total.xls')
sheet = 'sheet'
...
train_dataset = train_dataset[60:]


for j in range(len(train_dataset)):
    frame_dir = train_dataset[j] + 'Frame_RGB/'
    ...
    label_xls = xlrd.open_workbook_xls(train_dataset[j] + '0_gt_ppg_same_freq.xls')
    sheet = 'sheet'
    sheet_label = label_xls.sheet_by_name(sheet)
    rows_label = sheet_label.nrows

    node_map = np.zeros((128, len(frame_list), 3))
    ...
    for k in range(len(frame_list)):
        ...
        if img_path.endswith('.png'):
            ...
            img_face = face_recognition.load_image_file(img_path)

            face_landmarks_list = face_recognition.face_landmarks(img_face)

            if len(face_landmarks_list) != 0:
                have_face_path = img_path
            else:
                img_face = face_recognition.load_image_file(have_face_path)
                face_landmarks_list = face_recognition.face_landmarks(img_face)
            ...
            for i in range(len(new_landmark)):
                ...
                if y - (y_spn // 20) >= img.shape[0]:
                    y = img.shape[0] + (y_spn // 20) - 1

                if x - (x_spn // 20) >= img.shape[1]:
                    x = img.shape[1] + (x_spn // 20) - 1

                node_map[i, k, 0] = int(np.mean(img[y - (y_spn // 20): y + (y_spn // 20),
                                                x - (x_spn // 20): x + (x_spn // 20), 0]))
                node_map[i, k, 1] = int(np.mean(img[y - (y_spn // 20): y + (y_spn // 20),
                                                x - (x_spn // 20): x + (x_spn // 20), 1]))
                node_map[i, k, 2] = int(np.mean(img[y - (y_spn // 20): y + (y_spn // 20),
                                                x - (x_spn // 20): x + (x_spn // 20), 2]))

            img_face_ = img[y_min: y_min + y_spn, x_min: x_min + x_spn]
    ...
    cv2.imwrite(train_dataset[j] + 'node_map.png', node_map)
